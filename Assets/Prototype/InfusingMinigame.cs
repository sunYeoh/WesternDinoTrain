...
public class InfusingMinigame : MonoBehaviour
{
    ...
    private void Setup()
    {
        ...
        BuildUI();
        titleText.text = "ÁøÈ­ Á¶¸®  -  " + fusion.displayName + " [Àü¼³]";
        SetPhaseInfo();
    ...
    private void Update()
    {
        ...
        {
            UIManager.Instance?.ShowStatChange("ÁøÈ­ Á¶¸® Áß´Ü - µÎ ¿ä¸®´Â ±×´ë·Î ³²¾Ò´Ù");
            Destroy(gameObject);
            return;
        }
    ...
    private void SetPhaseInfo()
    {
        if (phase == 0)
            ...
            infoText.text = "2/2 [À¶ÇÕ ¾ÈÁ¤È­]  [Space] ¸¦ ´©¸£°í ¶¼¸ç ´«±ÝÀ» ÆÇÁ¤ ±¸°£¿¡ À¯Áö  (ÇöÀç " + score + "Á¡)";
        else
