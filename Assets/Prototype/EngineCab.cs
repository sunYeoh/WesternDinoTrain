...
public class EngineCab : MonoBehaviour
{
    ...
    /// <summary>v4: 이번 씬에서 작살로 낚은 횟수 (TutorialDirector 8단계 판정)</summary>
    public static int HarpoonRetrievals { get; private set; }

    /// <summary>v4: 이번 씬에서 레버를 당긴 횟수 - 전속/순항 어느 쪽이든 (TutorialDirector 9단계 판정)</summary>
    public static int LeverPulls { get; private set; }

    /// <summary>v4: 지금 길가에 흐르는 바위 수</summary>
    ...
    private bool InBattle()
    {
        return GameManager.Instance != null
            ...
            && (!WaveManager.TutorialGateActive || TutorialDirector.EngineCabUnlocked);   // v3.1: 프롤로그 조리 게이트 중에는 바위·작살·레버 없음 / v4: 견습 8단계부터는 켠다
    }
    ...
    {
        SpawnRockAt(-20f);   // v2: 앞(왼쪽)에서 나타난다
    }
    ...
    /// <summary>v4: 바위 하나를 지금 x 에 띄운다 (TutorialDirector 8단계 - 화면 안 왼쪽 -13 부터 흘러오게). 전투 중이 아니면 무시</summary>
    public static void SpawnRockNow(float x)
    {
        ...
        instance.nextRockTime = Time.time + GameBalance.RockSpawnIntervalMin;   // 자동 스폰은 한 박자 뒤로
    }
