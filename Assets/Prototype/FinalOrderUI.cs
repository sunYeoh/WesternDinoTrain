...
public class FinalOrderUI : MonoBehaviour
{
    ...
    private static string MissingText()
    {
        ...
        if (CountTier2Kinds() < t2Need)
            s += (s.Length > 0 ? " + " : "") + "전설 요리 " + (t2Need == 1 ? "한 접시" : t2Need + "종");
        return s;
    }
    ...
    private void Update()
    {
        ...
        if (choiceOpen)
        {
            ...
            else if (Input.GetKeyDown(KeyCode.F) || Input.GetKeyDown(KeyCode.Escape))
            {
                ...
                UIManager.Instance?.ShowStatChange("격파를 택했다 - [F] 독샘 요리로 밀어붙여라");
            }
            return;
        }
    ...
    private void Resolve()
    {
        ...
        else
        {
            ...
            if (boss.HasExtraGroggyPending)
                ...
                UIManager.Instance?.ShowStatChange("손이 떨렸다... 요리가 식었다. 이번 운행에는 기회가 없다 - 격파로 간다");
        }
    }
