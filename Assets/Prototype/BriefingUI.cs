...
public class BriefingUI : MonoBehaviour
{
    ...
    public class BriefDef
    {
        ...
        public string portrait = "";        // 초상 스프라이트 이름 (SpriteBank, 예: "e_ptera" / "ui_npc_spino"). 키와 같이 있으면 초상 1배 위 + 키 아래
    ...
    {
        // 다른 정지 창(정비소/열람) 위에 열리면 시간 소유권이 꼬인다 - 그쪽이 닫힐 때까지 기다린다.
        // v1.1: 증강 선택창 위에는 연다 (이미 멈춘 화면이라 pausedByMe 가 false - 시간은 그쪽이 돌려준다). 승격 카드 "증강 선택" 이 그 위에서 읽혀야 한다
    ...
    private void Fill(BriefDef def)
    {
        ...
        // 초상 / 키 카드 - v1.1 규칙: 초상 + 키 = 초상 1배(위) + 키 아래 / 초상만 = 2배 / 키만 = 큰 키 글자
        ...
        if (hasPortrait)
        {
            ...
            if (hasKey)
            {
                // 위쪽 100px 영역에 1배 (110 넘으면 반배, 그래도 크면 맞춰 줄임) - 실루엣 96x96 / 손님 스프라이트
            ...
            {
                // 월드 스프라이트(32ppu)는 픽셀 크기 그대로 2배 (200 을 넘으면 1배, 그래도 크면 200 에 맞춤) - 도트가 뭉개지지 않게 정수 배 우선
                float mx = Mathf.Max(w, h);
