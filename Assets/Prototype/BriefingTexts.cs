        ...
        switch (hintId)
        {
            ...
            case "first_route":
                return Make("차장", "분기 선로 - 위험과 보상의 교환", new string[] {
                    "두상 앞에서 선로가 갈라진다. 왼쪽 카드 [1~3] 또는 클릭으로 길을 골라라.",
                    "손님이 느는 길은 끝나면 증강을 한 번 더 고른다. 사고가 일찍 오는 길, 유물이 있는 길도 있다.",
                    "폐역에는 선대의 일지가 남아 있다 - [J] 로 읽는다.",
                    "짝수 웨이브 앞과 보스 직전에만 갈라진다. 고른 뒤 [Enter] 출발." }, "[1~3]", "선로 선택");
            case "first_item":
